...
void TB303Voice::prepare(double sampleRate)
{
    ...
    // Filter envelope: decay to 0 (no sustain) - classic 303 behavior
    filterEnvelope_.setSustainLevel(0.0f);
    // Amp envelope: sustain at full level while note is held
    ampEnvelope_.setSustainLevel(0.8f);
}
...
void TB303Voice::noteOn(int midiNote, bool accent, bool slide)
{
    ...
    if (!slide)
    {
        // Retrigger envelopes on non-slide notes
        filterEnvelope_.trigger();
        ampEnvelope_.trigger();
    }
    else
    {
        // On slide notes, retrigger filter envelope but keep amp sustained
        filterEnvelope_.trigger();
    }
...
{
    return noteActive_ || ampEnvelope_.isActive();
}
...
void TB303Voice::setDecay(float decayMs) { filterEnvelope_.setDecay(decayMs); ampEnvelope_.setDecay(decayMs * 2.0f); }
